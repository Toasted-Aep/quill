# vp10.ps1 - fourteenth screen run.  vp9.ps1 re-pathed to vp10data / vp10.
# Dot-source.
...
$script:QData   = "$QRoot\scratchpad\vp10data"
$script:QShots  = "$QRoot\scratchpad\vp10"
$script:QProbe  = "$QRoot\scratchpad\vp10\theme.log"
$script:QGeom   = "$QRoot\scratchpad\vp10\geom.log"
$script:QPid    = $null

if (-not (Test-Path $QShots)) { New-Item -ItemType Directory -Path $QShots | Out-Null }

Add-Type -Namespace VP10 -Name Desk -MemberDefinition @'
...
function Q-Desktop {
  $h = [VP10.Desk]::OpenInputDesktop(0, $false, 0x0100)
  if ($h -eq [IntPtr]::Zero) { return "<unreachable>" }
  $sb = New-Object System.Text.StringBuilder 256; $n = 0
  [VP10.Desk]::GetUserObjectInformation($h, 2, $sb, 256, [ref]$n) | Out-Null
  [VP10.Desk]::CloseDesktop($h) | Out-Null
  $sb.ToString()
}
...
# SendInput's absolute coordinates are 0..65535 over the virtual screen, so a
# requested x can LAND one physical pixel short.  Observed this run: Put 81,1087
# landed on 80,1087, and a 60 s track at that point then showed 1261 samples,
# ZERO cursor changes and idle rising monotonically - i.e. the injection's own
# quantisation, not a hand.  q.ps1's Put records the REQUESTED point, which
# turns that quantisation into a false abort.  Record where the cursor actually
# LANDED instead: the guard below stays exact-match strict against any later
# movement, and a landing that is not within 2 px of the request still throws.
...
# STRICT presence: throws on ANY displacement from where the cursor LANDED after
# our own last injection, and on the input desktop having moved out from under us.
function Q-Presence {
  $d = Q-Desktop
