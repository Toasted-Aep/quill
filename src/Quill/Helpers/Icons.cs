...
public static class Icons
{
    ...
    public const string UndoRound =
        // The user supplied a reference for this: a near-full THIN ring with a
        // gap over the top and a barbed head at one end of that gap. Superseding
        // the half annulus that was here - that was a defensive shape, chosen
        // because two 270-degree attempts had collapsed into a broken ring with
        // an unreadable stub. This one wraps 304 degrees and survives because
        // the band is thin (1.5 of a 17.5 diameter) and the head juts 1.55 past
        // BOTH radii, so the tip stays on the silhouette instead of being
        // swallowed by the band's own width.
        //
        // PathIcon data is filled, so the ring is an annulus - outer arc round,
        // head, inner arc back - not a stroke. Note the sweep flags: 1 is
        // CLOCKWISE in SVG's y-down space, and getting that backwards renders
        // the short arc and loses the ring entirely.
        // Generated by scratchpad/gen_undo.py; regenerate rather than hand-edit.
        "M16.11 4.82 A8.75 8.75 0 1 1 7.89 4.82 " +
        "L7.16 3.46 L11.03 4.61 L9.32 7.52 L8.6 6.15 " +
        "A7.25 7.25 0 1 0 15.4 6.15 Z";
